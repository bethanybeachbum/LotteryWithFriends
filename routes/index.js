  ...
  async.waterfall([
    ...
    function(token, user, done) {
      var smtpTransport = nodemailer.createTransport({
        service: 'Gmail', 
        auth: {
          user: 'coolbreezegf@gmail.com',
          pass: process.env.GMAILPW
        }
      });
      var mailOptions = {
        to: user.email,
        from: 'coolbreezegf@gmail.com',
...
router.get("/users/:id", function(req, res) {
  User.findById(req.params.id, function(err, foundUser) {
    ...
    Player.find().where('author.id').equals(foundUser._id).exec(function(err, players) {
      if(err) {
        req.flash("error", "Something went wrong.");
        res.redirect("/");
      }
      res.render("users/show", {user: foundUser, players: players});
    })
  });
});
